...
/**
 * The <code>JobDAO</code> class implements the job-related persistence operations.
 *
  ...
  implements IJobDAO
{
  ...
  /**
   * Constructs a new <code>JobDAO</code>.
   */
  public JobDAO() {}
        ...
        try (ResultSet rs = statement.executeQuery())
        {
          if (rs.next())
          {
            ...
            try (PreparedStatement updateStatement = connection.prepareStatement(lockJobSQL))
            {
              ...
              updateStatement.setString(4, job.getId());
  ...
  /**
   * Initialise the <code>JobDAO</code> instance.
   */
  @PostConstruct
  public void init()
  {
   ...
   * @throws DAOException
   */
  public void lockJob(String id, Job.Status status, String lockName)
    throws DAOException
  {
    try (Connection connection = dataSource.getConnection();
   ...
   * @throws DAOException
   */
  public void setJobStatus(String id, Job.Status status)
    throws DAOException
  {
    try (Connection connection = dataSource.getConnection())
    {
  ...
  private void setJobStatus(Connection connection, String id, Job.Status status)
  {
