 ...
 * the messaging infrastructure on a mobile device to download the queued message parts for the
 * device from the server-side messaging infrastructure.
 *
    ...
    for (Element messagePartElement : messagePartElements)
    {
      ...
      catch (Throwable e)
      {
        throw new MessagingException(
          "Failed to parse the WBXML for a message part associated with" + " the message part " +
            "download response", e);
      }
    }
  }
