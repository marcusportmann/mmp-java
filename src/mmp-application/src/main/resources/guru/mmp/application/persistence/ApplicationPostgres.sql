...
DROP TABLE IF EXISTS MMP.ROLE_TO_EXTERNAL_GROUP_MAP CASCADE;
...
CREATE UNIQUE INDEX MMP_INTERNAL_GROUPS_GROUPNAME_IX
...
CREATE INDEX MMP_EXTERNAL_GROUPS_USER_DIRECTORY_ID_IX
  ON MMP.EXTERNAL_GROUPS
  (USER_DIRECTORY_ID);

CREATE INDEX MMP_EXTERNAL_GROUPS_GROUPNAME_IX
  ON MMP.EXTERNAL_GROUPS
  (GROUPNAME);
...
COMMENT ON COLUMN MMP.EXTERNAL_GROUPS.USER_DIRECTORY_ID
...
CREATE TABLE MMP.ROLE_TO_EXTERNAL_GROUP_MAP (
  ROLE_ID            BIGINT NOT NULL,
  EXTERNAL_GROUP_ID  BIGINT NOT NULL,

  PRIMARY KEY (ROLE_ID, EXTERNAL_GROUP_ID),
  CONSTRAINT MMP_ROLE_TO_EXTERNAL_GROUP_MAP_ROLE_FK FOREIGN KEY (ROLE_ID) REFERENCES MMP.ROLES(ID) ON DELETE CASCADE,
  CONSTRAINT MMP_ROLE_TO_EXTERNAL_GROUP_MAP_EXTERNAL_GROUP_FK FOREIGN KEY (EXTERNAL_GROUP_ID) REFERENCES MMP.EXTERNAL_GROUPS(ID) ON DELETE CASCADE
);

CREATE INDEX MMP_ROLE_TO_EXTERNAL_GROUP_MAP_ROLE_ID_IX
  ON MMP.ROLE_TO_EXTERNAL_GROUP_MAP
  (ROLE_ID);

CREATE INDEX MMP_ROLE_TO_EXTERNAL_GROUP_MAP_EXTERNAL_GROUP_ID_IX
  ON MMP.ROLE_TO_EXTERNAL_GROUP_MAP
  (EXTERNAL_GROUP_ID);

COMMENT ON COLUMN MMP.ROLE_TO_EXTERNAL_GROUP_MAP.ROLE_ID
  IS 'The unique ID for the role';

COMMENT ON COLUMN MMP.ROLE_TO_EXTERNAL_GROUP_MAP.EXTERNAL_GROUP_ID
...
INSERT INTO MMP.USER_DIRECTORIES (ID, NAME, DESCRIPTION, USER_DIRECTORY_CLASS) VALUES
  (1, 'Internal User Directory', 'Internal User Directory', 'guru.mmp.application.security.InternalUserDirectory');

INSERT INTO MMP.USER_DIRECTORY_TO_ORGANISATION_MAP (ORGANISATION_ID, USER_DIRECTORY_ID) VALUES
  (1, 1);
...
INSERT INTO MMP.INTERNAL_USERS (ID, USER_DIRECTORY_ID, USERNAME, PASSWORD, TITLE, FIRST_NAMES, LAST_NAME, PHONE, FAX,
  ...
  (1, 1, 'Administrator', 'cMzZAHM41tgd07YnFiG5z5qX6gA=', '', '', '', '', '', '', '', null, null,
  'Administrator');
INSERT INTO MMP.INTERNAL_USERS (ID, USER_DIRECTORY_ID, USERNAME, PASSWORD, TITLE, FIRST_NAMES, LAST_NAME, PHONE, FAX,
  MOBILE, EMAIL, PASSWORD_ATTEMPTS, PASSWORD_EXPIRY, DESCRIPTION) VALUES
  (2, 1, 'test', 'cMzZAHM41tgd07YnFiG5z5qX6gA=', '', '', '', '', '', '', '', null, null,
  'Test User');

INSERT INTO MMP.INTERNAL_GROUPS (ID, USER_DIRECTORY_ID, GROUPNAME, DESCRIPTION) VALUES
  (1, 1, 'Administrators', 'Administrators');

INSERT INTO MMP.INTERNAL_USER_TO_INTERNAL_GROUP_MAP (USER_DIRECTORY_ID, INTERNAL_USER_ID, INTERNAL_GROUP_ID) VALUES (1, 1, 1);

INSERT INTO MMP.EXTERNAL_GROUPS (ID, USER_DIRECTORY_ID, GROUPNAME) VALUES (1, 1, 'Administrators');
...
INSERT INTO MMP.ROLE_TO_EXTERNAL_GROUP_MAP (ROLE_ID, EXTERNAL_GROUP_ID) VALUES (1, 1);
...
GRANT ALL ON TABLE MMP.ROLE_TO_EXTERNAL_GROUP_MAP TO dbuser;
