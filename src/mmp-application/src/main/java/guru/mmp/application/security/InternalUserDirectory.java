...
public class InternalUserDirectory extends UserDirectoryBase
{
  ...
  protected void buildStatements(String schemaPrefix)
  {
    ...
    getFunctionCodesForUserIdSQL =
      ...
      + "ROLE_TO_EXTERNAL_GROUP_MAP RTEGM ON RTEGM.ROLE_ID = FTRM.ROLE_ID INNER JOIN "
      + schemaPrefix + "EXTERNAL_GROUPS EG ON EG.ID = RTEGM.EXTERNAL_GROUP_ID INNER JOIN "
      + schemaPrefix + "INTERNAL_GROUPS IG ON IG.USER_DIRECTORY_ID = EG.USER_DIRECTORY_ID"
      + "     AND UPPER(IG.GROUPNAME) = UPPER(EG.GROUPNAME) INNER JOIN " + schemaPrefix
