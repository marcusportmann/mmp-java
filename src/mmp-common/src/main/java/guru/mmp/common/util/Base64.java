...
public class Base64
{
  ...
  /**
   * Attempts to decode Base64 data and deserialize a Java Object within. Returns <tt>null if there
   * was an error.
   *
  ...
  /**
   * Encodes a string in Base64 notation with line breaks after every 75 Base64 characters. Of
   * course you probably only need to encode a string if there are non-ASCII characters in it such
   * as many non-English languages.
   *
  ...
  private static int decode4to3(byte[] source, int srcOffset, byte[] destination, int destOffset)
  {
    ...
    else if (source[srcOffset + 3] == EQUALS_SIGN)
    {
      ...
      int outBuff = ((DECODABET[source[srcOffset]] & 0xFF) << 18)
          | ((DECODABET[source[srcOffset + 1]] & 0xFF) << 12) | ((DECODABET[source[srcOffset + 2]]
          & 0xFF) << 6);
