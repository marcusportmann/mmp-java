...
public class WebServiceClientSecurityHelper
{
   ...
   */
  private static MutualSSLSocketFactory clientSSLSocketFactory;
  ...
  private static SSLSocketFactory getMutualSSLSocketFactory()
  {
    try
    {
      if (clientSSLSocketFactory == null)
      {
        ...
        clientSSLSocketFactory = new MutualSSLSocketFactory(ApplicationSecurityContext.getContext()
    ...
    try
    {
      ...
      if (portName == null)
      {
        throw new WebServiceClientSecurityException("Failed to determine the port name using the"
            + " @WebEndpoint annotation on one of "
